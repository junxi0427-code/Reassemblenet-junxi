...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        if self.use_geometry_only or self.use_geometry_and_global_texture or self.use_geometry_and_local_texture or self.use_geometry_global_local_texture:
            self.geometry_emb = nn.Linear(2, self.model_channels)
        ...
        if self.use_global_texture_only or self.use_geometry_and_global_texture or self.use_local_and_global_texture or self.use_geometry_global_local_texture:
            self.global_texture_emb = nn.Linear(512, self.model_channels)
        ...
        if self.use_local_texture_only or self.use_geometry_and_local_texture or self.use_local_and_global_texture or self.use_geometry_global_local_texture:
            self.local_texture_emb = nn.Linear(512, self.model_channels)
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        if self.use_geometry_only or self.use_geometry_and_global_texture or self.use_geometry_and_local_texture or self.use_geometry_global_local_texture:
            geometric_embedding = self.geometry_emb(kwargs['geom_feats'].float())
        else:
            geometric_embedding = 0

        if self.use_global_texture_only or self.use_geometry_and_global_texture or self.use_local_and_global_texture or self.use_geometry_global_local_texture:
            global_texture_embedding = self.global_texture_emb(kwargs['global_texture_feats'].float())
        else:
            global_texture_embedding = 0

        if self.use_local_texture_only or self.use_geometry_and_local_texture or self.use_local_and_global_texture or self.use_geometry_global_local_texture:
            local_texture_embedding = self.local_texture_emb(kwargs['local_texture_feats'].float())
        else:
            local_texture_embedding = 0
        
        
        out = input_emb + cond_emb + image_embedding + geometric_embedding + global_texture_embedding + local_texture_embedding + time_emb.repeat((1, input_emb.shape[1], 1))
